...
class SimpleNet(nn.Module):
    def __init__(
        ...
        ######## TODO ########
        # Initialize ModuleLists for layers and activations
        self.time_linears = nn.ModuleList()
        self.activations = nn.ModuleList()

        # Input layer
        self.time_linears.append(TimeLinear(dim_in, dim_hids[0], num_timesteps))
        self.activations.append(nn.ReLU())

        # Hidden layers
        for i in range(len(dim_hids) - 1):
            self.time_linears.append(TimeLinear(dim_hids[i], dim_hids[i+1], num_timesteps))
            self.activations.append(nn.ReLU())

        # Output layer (no activation function)
        self.time_linears.append(TimeLinear(dim_hids[-1], dim_out, num_timesteps))
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        for i in range(len(self.activations)):
            x = self.time_linears[i](x, t)
            x = self.activations[i](x)
        
        # Output layer (no activation function)
        x = self.time_linears[-1](x, t)     
        ######################
        return x
